...
class Bpgan_GAN_Q_Model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.quantize_type == 'scalar':
            ...
            temp = 1
            self.Q = networks.quantizer(center=center,Temp=temp).cuda()
        elif opt.quantize_type == 'vector':
            center = torch.Tensor(opt.n_cluster,4).cuda()
            temp = 1
            self.Q = networks.vector_quantizer(center=center,Temp=temp).cuda()
